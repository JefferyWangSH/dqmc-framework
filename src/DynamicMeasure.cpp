...
void Measure::DynamicMeasure::initial(const Model::Hubbard &hubbard) {
    ...
    this->obs_bin_g_kt.reserve(nbin);
    this->obs_bin_rho_s.reserve(nbin);
    this->obs_bin_sign.reserve(nbin);

    this->obs_bin_gt0_up.reserve(nbin);
    this->obs_bin_g0t_up.reserve(nbin);
    this->obs_bin_gtt_up.reserve(nbin);
    this->obs_bin_gt0_dn.reserve(nbin);
    this->obs_bin_g0t_dn.reserve(nbin);
    this->obs_bin_gtt_dn.reserve(nbin);

    for (int bin = 0; bin < this->nbin; ++bin) {
        this->obs_bin_g_kt.emplace_back(hubbard.lt, 0.0);
        this->obs_bin_rho_s.emplace_back(0.0);
        this->obs_bin_sign.emplace_back(0.0);

        this->obs_bin_gt0_up.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
        this->obs_bin_g0t_up.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
        this->obs_bin_gtt_up.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
        this->obs_bin_gt0_dn.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
        this->obs_bin_g0t_dn.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
        this->obs_bin_gtt_dn.emplace_back(hubbard.lt, Eigen::MatrixXd::Zero(hubbard.ls, hubbard.ls));
    }

    this->obs_mean_rho_s = 0.0;
    this->obs_err_rho_s = 0.0;

    this->obs_mean_sign = 0.0;
    this->obs_err_sign = 0.0;
    this->tmp_sign = 0.0;

    this->obs_mean_g_kt.reserve(hubbard.lt);
    this->obs_err_g_kt.reserve(hubbard.lt);
    ...
    for (int l = 0; l < hubbard.lt; ++l) {
        this->obs_mean_g_kt.emplace_back(0.0);
        this->obs_err_g_kt.emplace_back(0.0);
    ...
    this->obs_bin_g_kt.shrink_to_fit();
    this->obs_mean_g_kt.shrink_to_fit();
    this->obs_err_g_kt.shrink_to_fit();
    this->obs_bin_rho_s.shrink_to_fit();
    this->obs_bin_sign.shrink_to_fit();

    this->obs_bin_gt0_up.shrink_to_fit();
    this->obs_bin_g0t_up.shrink_to_fit();
    this->obs_bin_gtt_up.shrink_to_fit();
    this->obs_bin_gt0_dn.shrink_to_fit();
    this->obs_bin_g0t_dn.shrink_to_fit();
    this->obs_bin_gtt_dn.shrink_to_fit();
    ...
    assert( this->obs_bin_gt0_up.size() == this->nbin );
    assert( this->obs_bin_g0t_up.size() == this->nbin );
    assert( this->obs_bin_gtt_up.size() == this->nbin );
    assert( this->obs_bin_gt0_dn.size() == this->nbin );
    assert( this->obs_bin_g0t_dn.size() == this->nbin );
    assert( this->obs_bin_gtt_dn.size() == this->nbin );
...
void Measure::DynamicMeasure::clear() {
    this->obs_bin_g_kt.clear();
    this->obs_bin_rho_s.clear();
    this->obs_bin_sign.clear();

    this->obs_bin_gt0_up.clear();
    this->obs_bin_g0t_up.clear();
    this->obs_bin_gtt_up.clear();
    this->obs_bin_gt0_dn.clear();
    this->obs_bin_g0t_dn.clear();
    this->obs_bin_gtt_dn.clear();

    this->obs_mean_g_kt.clear();
    this->obs_err_g_kt.clear();
...
void Measure::DynamicMeasure::clear(const Model::Hubbard &hubbard) {
...
void Measure::DynamicMeasure::normalizeStats(const Model::Hubbard &hubbard) {
    for (int l = 0; l < hubbard.lt; ++l) {
        this->tmp_gt0_tau_up[l] /= this->n_time_displaced;
        this->tmp_g0t_tau_up[l] /= this->n_time_displaced;
        this->tmp_gtt_tau_up[l] /= this->n_time_displaced;
        this->tmp_gt0_tau_dn[l] /= this->n_time_displaced;
        this->tmp_g0t_tau_dn[l] /= this->n_time_displaced;
        this->tmp_gtt_tau_dn[l] /= this->n_time_displaced;
    }
...
void Measure::DynamicMeasure::write_Stats_to_bins(const int &bin, const Model::Hubbard &hubbard) {
    // TODO: CHECK IT OUT
    for (int l = 0; l < hubbard.lt; ++l) {
        this->obs_bin_gt0_up[bin][l] = this->tmp_gt0_tau_up[l] / this->tmp_sign;
        this->obs_bin_g0t_up[bin][l] = this->tmp_g0t_tau_up[l] / this->tmp_sign;
        this->obs_bin_gtt_up[bin][l] = this->tmp_gtt_tau_up[l] / this->tmp_sign;
        this->obs_bin_gt0_dn[bin][l] = this->tmp_gt0_tau_dn[l] / this->tmp_sign;
        this->obs_bin_g0t_dn[bin][l] = this->tmp_g0t_tau_dn[l] / this->tmp_sign;
        this->obs_bin_gtt_dn[bin][l] = this->tmp_gtt_tau_dn[l] / this->tmp_sign;
    }
    this->obs_bin_sign[bin] = this->tmp_sign;
}
...
void Measure::DynamicMeasure::analyse_Dynamical_Corr(const int &bin, const Model::Hubbard &hubbard) {
    for (int l = 0; l < hubbard.lt; ++l) {
        ...
        const Eigen::MatrixXd gt0 = 0.5 * (this->obs_bin_gt0_up[bin][l] + this->obs_bin_gt0_dn[bin][l]);
        ...
        this->obs_bin_g_kt[bin][l] = tmpFourier;
    }
}
...
void Measure::DynamicMeasure::analyse_Rho_S(const int &bin, const Model::Hubbard &hubbard) {
    ...
    for (int l = 0; l < hubbard.lt; ++l) {
        ...
        const Eigen::MatrixXd gt0_up = this->obs_bin_gt0_up[bin][l];
        const Eigen::MatrixXd g0t_up = this->obs_bin_g0t_up[bin][l];
        const Eigen::MatrixXd gtt_up = this->obs_bin_gtt_up[bin][l];
        const Eigen::MatrixXd g00_up = this->obs_bin_gtt_up[bin][hubbard.lt - 1];
        const Eigen::MatrixXd gt0_dn = this->obs_bin_gt0_dn[bin][l];
        const Eigen::MatrixXd g0t_dn = this->obs_bin_g0t_dn[bin][l];
        const Eigen::MatrixXd gtt_dn = this->obs_bin_gtt_dn[bin][l];
        const Eigen::MatrixXd g00_dn = this->obs_bin_gtt_dn[bin][hubbard.lt - 1];
    ...
    this->obs_bin_rho_s[bin] = 0.25 * tmp_rho_s;
}
...
void Measure::DynamicMeasure::analyse_timeDisplaced_Stats(const Model::Hubbard &hubbard) {
    ...
    for (int l = 0; l < hubbard.lt; ++l) {
        this->obs_mean_g_kt[l] = 0.0;
        this->obs_err_g_kt[l] = 0.0;
    }
    this->obs_mean_rho_s = 0.0;
    this->obs_err_rho_s = 0.0;
    this->obs_mean_sign = 0.0;
    ...
    for (int bin = 0; bin < this->nbin; ++bin) {
        for (int l = 0; l < hubbard.lt; ++l) {
            this->obs_mean_g_kt[l] += this->obs_bin_g_kt[bin][l];
            this->obs_err_g_kt[l] += this->obs_bin_g_kt[bin][l] * this->obs_bin_g_kt[bin][l];
        }
        this->obs_mean_rho_s += this->obs_bin_rho_s[bin];
        this->obs_err_rho_s += this->obs_bin_rho_s[bin] * this->obs_bin_rho_s[bin];
        this->obs_mean_sign += this->obs_bin_sign[bin];
        this->obs_err_sign += this->obs_bin_sign[bin] * this->obs_bin_sign[bin];
    }

    this->obs_mean_sign /= this->nbin;
    this->obs_err_sign /= this->nbin;
    this->obs_err_sign = pow(this->obs_err_sign - pow(this->obs_mean_sign, 2), 0.5) / pow(this->nbin - 1, 0.5);
    for (int l = 0; l < hubbard.lt; ++l) {
        this->obs_mean_g_kt[l] /= this->nbin;
        this->obs_err_g_kt[l] /= this->nbin;
        this->obs_err_g_kt[l] = pow(this->obs_err_g_kt[l] - pow(this->obs_mean_g_kt[l], 2), 0.5) / pow(this->nbin - 1, 0.5);
    }
    this->obs_mean_rho_s /= this->nbin;
    this->obs_err_rho_s /= this->nbin;
    this->obs_err_rho_s = pow(this->obs_err_rho_s - pow(this->obs_mean_rho_s, 2), 0.5) / pow(this->nbin - 1, 0.5);
}
