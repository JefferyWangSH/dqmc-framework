...
template<> void Observable::Observable<Observable::ScalarType>::calculate_error_bar() {
...
template<> void Observable::Observable<Observable::VectorType>::calculate_error_bar() {
    for (const auto& bin_data : this->m_bin_data) {
...
template<> void Observable::Observable<Observable::MatrixType>::calculate_error_bar() {
    for (const auto& bin_data : this->m_bin_data) {
        this->m_error_bar += bin_data.array().square().matrix();
    }
    this->m_error_bar /= this->size_of_bin();
