...
void detQMC::set_Momentum_q(double qx, double qy) {
    ...
    eqtimeMeasure.q = (vecXd(2) << qx, qy).finished();
    dynamicMeasure.q = (vecXd(2) << qx, qy).finished();
}
    ...
    if (bool_measure_dynamic) {
        ...
        for (int l = 1; l <= hubb.lt; ++l) {
            outfile << std::setw(15) << l
                    << std::setw(15) << dynamicMeasure.obs_mean_gt0_k[l-1]
                    << std::setw(15) << dynamicMeasure.obs_err_gt0_k[l-1]
                    << std::setw(15) << dynamicMeasure.obs_err_gt0_k[l-1] / dynamicMeasure.obs_mean_gt0_k[l-1]
                    << std::endl;
        }
        outfile.close();
