...
//Eigen::MatrixXd Hubbard::make_B_l(int l, int sigma) {
//    /*
...
//
//    Eigen::MatrixXd r;
//    if (checkerboard.is_checker_board()) {
//        r = Eigen::MatrixXd::Identity(ls, ls);
//        checkerboard.mult_expK_from_left(r);
//    }
//    else { r = checkerboard.exp_dtK;}
//
//    for (int i = 0; i < ls; ++i) {
...
void Hubbard::mult_B_from_left(Eigen::MatrixXd &A, int l, int sigma) {
    /*
    ...
    const int tau = (l==0)? lt-1 : l-1;
    const int eff_sigma = (u_is_attractive)? +1 : sigma;
    checkerboard.mult_expK_from_left(A);
    for (int i = 0; i < ls; ++i) {
        A.row(i) *= exp(+ eff_sigma * alpha * s(i, tau));
    }
}

void Hubbard::mult_B_from_right(Eigen::MatrixXd &A, int l, int sigma) {
    /*
    ...
    checkerboard.mult_expK_from_right(A);
}

void Hubbard::mult_invB_from_left(Eigen::MatrixXd &A, int l, int sigma) {
    /*
    ...
    checkerboard.mult_inv_expK_from_left(A);
}

void Hubbard::mult_invB_from_right(Eigen::MatrixXd &A, int l, int sigma) {
    /*
    ...
    const int tau = (l==0)? lt-1 : l-1;
    const int eff_sigma = (u_is_attractive)? +1 : sigma;
    checkerboard.mult_inv_expK_from_right(A);
    for (int i = 0; i < ls; ++i) {
        A.col(i) *= exp(- eff_sigma * alpha * s(i, tau));
    }
}

void Hubbard::mult_transB_from_left(Eigen::MatrixXd &A, int l, int sigma) {
    /*
    ...
    checkerboard.mult_trans_expK_from_left(A);
}
